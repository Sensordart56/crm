...
function TaskOnePage() {
  ...
  return (
    <div className="print-sheet">
      ...
      <section className="top-three-section"><div className="section-heading"><div><span className="eyebrow">Explicit shortlist</span><h2>Top three, with the trade-offs visible</h2></div><span className="muted">Priority: Chris Ortega · evidence-led tie-breakers</span></div><div className="top-three-grid">{rankedProfessionals.slice(0, 3).map((person) => <article className="top-three-card" key={person.id}><div className="top-three-head"><span className="top-three-rank">0{rankOf(person.id)}</span><strong>{totalScore(person)}<small>/20</small></strong></div><h3>{person.name}</h3><p className="top-three-role">{person.role} · {person.company}</p><div><strong>Why shortlisted</strong><span>{person.whyValuable}</span></div><div><strong>Trade-off to test</strong><span>{person.assumed[0]}</span></div></article>)}</div></section>
      ...
      <section className="table-section"><div className="section-heading"><div><span className="eyebrow">Ranked table</span><h2>Evidence before enthusiasm</h2></div><span className="muted">Click a row to inspect verified vs assumed</span></div><div className="table-wrap"><table className="research-table"><thead><tr><th>Rank</th><th>Professional</th><th>Public signal & community value</th><th>Potential contribution</th><th>Space</th><th>Relevance score</th><th>Contribution score</th><th>Member value score</th><th>Evidence score</th><th>Total</th></tr></thead><tbody>{rankedProfessionals.map((person) => { const rank = rankOf(person.id); return <tr key={person.id} className={person.id === priorityInvitee.id ? 'priority-row' : ''}><td className="rank-cell">{rank === 1 ? '01' : `0${rank}`}</td><td><div className="person-cell"><div className="avatar">{person.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</div><div><strong>{person.name}</strong><span>{person.role}<br />{person.company}</span><button type="button" className="detail-toggle" onClick={() => setExpanded(expanded === person.id ? null : person.id)}>{expanded === person.id ? 'Hide evidence' : 'Show evidence'}</button></div></div>{expanded === person.id ? <div className="evidence-drawer"><div><strong>Verified</strong><ul>{person.verified.map((item) => <li key={item}>{item}</li>)}</ul></div><div><strong>Assumed / to test</strong><ul>{person.assumed.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="source-stack">{person.sources.map((source) => <SourceLink key={source.id} {...source} />)}</div></div> : null}</td><td className="signal-cell"><strong>Public signal</strong><p>{person.signal}</p><strong>Why FoF may be valuable</strong><p>{person.whyValuable}</p></td><td className="contribution-cell"><p>{person.contribution}</p></td><td><Pill tone="teal">{person.space}</Pill></td><td>{person.scores.communityRelevance}/5</td><td>{person.scores.potentialContribution}/5</td><td>{person.scores.likelyMemberValue}/5</td><td>{person.scores.evidenceStrength}/5</td><td><strong className="total-score">{totalScore(person)}</strong><small>/20</small></td></tr>})}</tbody></table></div></section>
